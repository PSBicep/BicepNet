    ...
    public BicepConfigInfo GetConfigurationInfo(BicepConfigScope mode, Uri sourceFileUri)
    {
        ...
        switch (mode)
        {
            ...
            case BicepConfigScope.Merged:
                ...
                return new BicepConfigInfo(config.ConfigurationPath ?? "Default", config.ToUtf8Json());
            case BicepConfigScope.Local:
                config = GetConfiguration(sourceFileUri);
                if (config.ConfigurationPath is not null)
                {
                    using var fileStream = fileSystem.FileStream.New(config.ConfigurationPath, FileMode.Open, FileAccess.Read);
                    var configString = JsonElementFactory.CreateElementFromStream(fileStream).ToString();
                    return new BicepConfigInfo(config.ConfigurationPath, configString);
                }
